var Hotpot = (function () {
  var isLogging = true;
  const itemTypes = { design: 0, text: 1, graphic: 2 };
  ...
  const sizeEventTypes = { changeWidth: 0, changeHeight: 1 };
  ...
  const localStorageKeys = { referrerId: "ai.hotpot.user.referrerId" };
  const urlParameters = { referrerId: "r-id" };
...
function openHotpotImageHelper(hotpotUrl, imageUrl, referrer) {
  let imageType = "png";
  let queryString =
    "?s=" + referrer + "&imageUrl=" + imageUrl + "&imageType=" + imageType;
...
function getImageSize(imageURL, callback) {
  ...
  if (svgDoc) {
    ...
    if (!viewBox) {
      debugError(
        "Error getting image size: SVG without viewBox. Image URL: " + imageURL
      );
      if (callback) {
        callback(null, null);
      }
...
function sizeIframeToContent(item) {
  ...
  if (iframe.length == 0) {
    debugError(
      "Error sizing iframe to content: no iframe found for " + printName(item)
    );
    return;
  }
...
async function downloadHttpUrl(imageUrl, filename) {
  try {
    let resp = await fetch(imageUrl, { cache: "no-cache" });
...
async function downloadImageListAsZipFile(imageUrlList, filename) {
  ...
  MessageClass.showMessage(
    `Downloading ${numImages} images ... may take a few minutes`
  );
  const max_errors = 10;
  let num_errors = 0;
  const downloadPromises = imageUrlList.map(async (url, index) => {
    try {
      ...
      if (num_errors >= max_errors) {
        MessageClass.showMessage(
          "Your network has an error. Please wait 3-5 minutes and try again. Do not refresh the page.",
          null,
          true
        );
  ...
  MessageClass.showMessage(
    "Preparing zip file ... may take 2-5 minutes",
    null,
    true
  );
...
function scrollToElement(element, dur_ = 400) {
  const dur = dur_;
  ...
  const scrollTop = scrollContainer.is("html, body")
    ? 0
    : scrollContainer.scrollTop();
  const containerTop = scrollContainer.is("html, body")
    ? 0
    : containerOffsetTop;
  const targetScrollTop = elementOffsetTop + scrollTop - containerTop;
  scrollContainer.animate({ scrollTop: targetScrollTop }, dur);
}
function scrollToTopOf(element, dur_) {
  const dur = dur_ || 400;
  $(element).animate({ scrollTop: 0 }, dur);
}
...
function camelCaseToWords(camelCaseWord) {
  return camelCaseWord.replace(/([A-Z])/g, " $1").replace(/^./, function (str) {
...
function hasExtension(string, extensionList) {
  if (!string || !extensionList || !extensionList.length) {
    ...
    var matches = extensionList.filter((x) =>
      lowerCaseString.endsWith(x.toLowerCase())
    );
...
function getPluralString(num, nounParam) {
  let exceptionList = { person: "people", man: "men", woman: "women" };
  let noun = nounParam;
...
function dataUrlToBase64Image(dataUrl) {
  if (!dataUrl) {
    ...
    let base64String = dataUrl.replace(
      /^data:(image\/(\w+)|application\/octet-stream);base64,/,
      ""
    );
    return base64String;
  }
...
function didFindUnsupportedBrowser(isMobile) {
  ...
  var locationURL =
    window.location.protocol + "//" + window.location.host + fallbackURL;
...
async function didMakeCreditPayment() {
  ...
  logEvent("Made payment", LogCategoryName);
}
...
var ClipboardClass = (function () {
  ...
  function copyText(text) {
    var tempElem = $(
      '<textarea style="position: absolute; top: -8888px; left: -8888px">'
    );
...
var ModalClass = (function () {
  ...
  return { showModalPage: showModalPage, hideModal: hideModal };
})();
var MessageClass = (function () {
  ...
  function showTooltip(elementBox) {
    const tooltipBox = elementBox.closest(".tooltipBox");
  ...
  function showMessage(message, dur, willReplace) {
...
class FormClass {
  ...
  static async imageUrlToFile(url, type) {
    const response = await fetch(url);
    const data = await response.blob();
    const metadata = { type: type || "image/png" };
  ...
  static getTextDataFromFile(file) {
    ...
    reader.onload = function (e) {
      let data = { filename: file.name, text: reader.result };
...
let SiteMenuClass = (function () {
  ...
  function init() {
    SiteMenu = $("#siteMenu");
    SiteMenu.find(".category.root .header").on(
  ...
  return { showCategory: showCategory, show: show, hide: hide, init: init };
})();
...
let DownloadOverlayClass = (function () {
  ...
  return { init: init, show: show, hide: hide, download: download };
})();
